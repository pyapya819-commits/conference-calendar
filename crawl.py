...
def crawl_hypertension():
    ...
    for page in range(1, 10):
        url = f'{base}/symposium/symposium?page={page}'
        soup = get_soup(url)
        if not soup:
            break
...
def crawl_neurology():
    ...
    for page in range(1, 10):
        url = f'{base}/kr/board_list/do-schedule?year={year}&page={page}'
        soup = get_soup(url)
        if not soup:
            break
        ...
        for item in items:
            ...
            if end_match:
                ed = end_match.group(1)
                end = f"{year:04d}-{int(mo):02d}-{int(ed):02d}"

            # 이름 추출
            name_match = re.search(r'\d+월\s*\d+\(.+?\)\s*(?:~\s*\d+\(.+?\)\s*)?(.+?)(?:장소|자세히|$)', text)
            ...
            intl_kw = ['국제','International','WFN','AAN','EAN','WSC']
            event_type = 'intl' if any(k.lower() in name.lower() for k in intl_kw) else 'domestic'
...
def crawl_nephrology():
    ...
    for month in range(1, 13):
        url = f'{base}/bbs/index.php?code=schedule&year={year}&month={month}'
        soup = get_soup(url)
        if not soup:
            continue

        items = soup.select('li, .schedule_list li')
        for item in items:
            text = item.get_text(separator=' ', strip=True)
            if not text or len(text) < 10:
                continue
            ...
            if end_match:
                ...
                end = f"{year:04d}-{int(emo):02d}-{int(ed):02d}"

            # 이름 추출 (날짜 뒤 텍스트)
            name_match = re.search(r'\d{2}\.\d{2}(?:\(.+?\))?\s*(?:~\s*\d{2}\.\d{2}(?:\(.+?\))?)?\s*(.+?)(?:\s*-\s*장소|\s*$)', text)
            name = name_match.group(1).strip() if name_match else text[:60]
            if not name or len(name) < 5:
                continue
            ...
            intl_kw = ['ISN','ERA','ASN','International','국제','JSN','JSDT']
            event_type = 'intl' if any(k.lower() in name.lower() for k in intl_kw) else 'domestic'

            ev = make_event(name, start, end, location, 'endo', event_type, '대한신장학회', '')
            if ev:
                results.append(ev)
                print(f"  ✅ {name} ({start})")
...
def crawl_orthopedics():
    ...
    for code, label in [('event1', 'domestic'), ('event2', 'intl')]:
        for page in range(1, 5):
            url = f'{base}/bbs/?code={code}&page={page}'
            soup = get_soup(url)
            if not soup:
                break
...
def crawl_rehabilitation():
    ...
    results = []
    for page in range(1, 5):
        url = f'https://www.karm.or.kr/bbs/?code=schedule&page={page}'
        soup = get_soup(url)
        if not soup:
            break
...
def crawl_neurosurgery():
    ...
    results = []
    for page in range(1, 5):
        url = f'https://www.neurosurgery.or.kr/conference/?page={page}'
        soup = get_soup(url)
        if not soup:
            break
...
def crawl_endocrinology():
    ...
    results = []
    for page in range(1, 5):
        url = f'https://endocrinology.or.kr/event/schedule/?page={page}'
        soup = get_soup(url)
        if not soup:
            break
...
def crawl_cardiology():
    ...
    results = []
    for page in range(1, 5):
        url = f'https://circulation.or.kr/workshop/sub1.php?page={page}'
        soup = get_soup(url)
        if not soup:
            break
